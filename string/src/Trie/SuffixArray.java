...
public class SuffixArray {
	public static void main(String[] args) {
		String s = "banana";
		//s문자열의 모든 접미사 생성 - 실제 문자열을 생성화지 않고 시작 문자의 위치를 기록
		Integer[] sa = new Integer[s.length()];
		for(int i=0;i<sa.length;i++)
			sa[i] = i;
		//sa[i]번째 부터 시작하는 문자열들이 각각 사전순으로 오름차순 되도록 정렬.
		...
		{
			for(int j=0;j<Math.min(s.substring(sa[i-1]).length(), s.substring(sa[i]).length());j++)
			{
				if(s.substring(sa[i-1]).charAt(j) != s.substring(sa[i]).charAt(j)) break;
				LCP[i]++;
			}
		}
		...
		for(int i=0;i<sa.length;i++) {
			...
			if(k- (str.length()-LCP[i]) > 0 ) k=k-(str.length()-LCP[i]);
			else {
